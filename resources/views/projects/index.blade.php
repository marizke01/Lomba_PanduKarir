...
<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-50">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 py-6 sm:py-8 space-y-5">
            ...
            <form id="projectSearchForm" method="GET" action="{{ route('projectlab.index') }}" class="space-y-3">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full pl-9 pr-28 py-2.5 text-[12px] rounded-xl border border-slate-200
                        ...
                        <button
                            ...
                            class="absolute right-1.5 top-1/2 -translate-y-1/2 inline-flex items-center justify-center
                                px-4 py-2 rounded-xl bg-violet-600 text-white text-xs font-medium hover:bg-violet-700">
                            Cari
                        </button>
                    </div>
                </div>
            ...
            {{-- Auto reset saat input dikosongkan --}}
            <script>
            ...
            input.addEventListener('input', () => {
                ...
                // kalau sebelumnya ada isi, lalu jadi kosong -> submit untuk kembali ke semua project
                if (lastValue.trim() !== '' && v === '') {
                form.submit();
                }
            ...
            // Enter tetap jalan normal submit
            });
            </script>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                @forelse(($projects ?? []) as $p)
                    @php
                        $isDone = in_array($p['slug'], $submittedSlugs ?? []);
                    @endphp
                    @php
                        $thumb = $p['thumbnail'] ?? '';
                    ...
                    <a href="{{ route('projectlab.show', $p['slug']) }}"
                       class="group bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                        ...
                        <div class="relative aspect-[16/9] bg-slate-100 overflow-hidden">
                            ...
                            />
                             @if($isDone)
                                <span class="absolute top-3 right-3 text-[11px] px-3 py-1 rounded-full bg-emerald-600 text-white shadow">
                                    ✓ Sudah dikerjakan
                                </span>
                            @endif
