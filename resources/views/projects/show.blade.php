                ...
                <div class="lg:sticky lg:top-6 self-start">
                    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
                        ...
                        <form method="POST"
                            ...
                            {{-- Link --}}
                            <div>
                                <label class="block text-[12px] text-slate-600 mb-1">Link hasil (GitHub/Figma/Canva/dll)</label>
                                <input type="url" name="submission_url"
                                    value="{{ old('submission_url', optional($assignment)->submission_url) }}"
                                    placeholder="https://github.com/username/repo"
                                    class="w-full text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400">
                            </div>

                            {{-- Dokumen --}}
                            <div>
                                <label class="block text-[12px] text-slate-600 mb-1">Upload dokumen (PDF/DOC)</label>
                                <input type="file" name="submission_file"
                                    accept=".pdf,.doc,.docx"
                                    class="w-full text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white">
                                @if(!empty($assignment?->submission_file_path))
                                    <p class="text-[11px] text-slate-500 mt-1">
                                        File saat ini:
                                        <a class="text-violet-700 underline" target="_blank"
                                        href="{{ asset('storage/'.$assignment->submission_file_path) }}">Download</a>
                                    </p>
                                @endif
                                <p class="text-[11px] text-slate-400 mt-1">Kalau tidak upload baru, file lama tetap dipakai.</p>
                            </div>

                            {{-- Gambar --}}
                            <div>
                                <label class="block text-[12px] text-slate-600 mb-1">Upload gambar (screenshot)</label>
                                <input type="file" name="submission_image"
                                    accept="image/*"
                                    class="w-full text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white">
                                @if(!empty($assignment?->submission_image_path))
                                    <div class="mt-2">
                                        <img src="{{ asset('storage/'.$assignment->submission_image_path) }}"
                                            class="rounded-xl border border-slate-200 max-h-48"
                                            alt="Screenshot hasil">
                                    </div>
                                @endif
                            </div>
                            ...
                            {{-- Catatan --}}
                            <div>
                                <label class="block text-[12px] text-slate-600 mb-1">Catatan (opsional)</label>
                                <textarea name="notes" rows="4"
                                        class="w-full text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400"
                                        placeholder="Tulis ringkasan hasil, kendala, yang kamu pelajari...">{{ old('notes', optional($assignment)->notes) }}</textarea>
                            </div>

                            <button type="submit"
                                    class="w-full inline-flex items-center justify-center px-4 py-3 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700">
                                Kirim
                            </button>

                            <p class="text-[11px] text-slate-500 text-center">
                                Kamu boleh update submit. Kiriman baru akan menggantikan yang lama.
                            </p>
                        </form>
