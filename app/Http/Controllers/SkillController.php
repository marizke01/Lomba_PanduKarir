...
class SkillController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($userId) {
            foreach ($courses as $c) {
                $slugCourse = $c['slug'];
                $total = count($c['modules'] ?? []);

                if ($total === 0) continue;

                $completedCount = ModuleProgress::where('user_id', $userId)
                    ->where('course_slug', $slugCourse)
                    ->where('is_completed', true)
                    ->count();

                if ($completedCount >= $total) {
                    $completedCourseSlugs[] = $slugCourse;
                }
            }
        }
    ...
    public function completeModule(Request $request, string $slug, int $moduleIndex)
    {
        ...
        $modules = $course['modules'] ?? [];
        $totalModules = count($modules);
        abort_if($moduleIndex < 0 || $moduleIndex >= $totalModules, 404);

        $userId = Auth::id();

        // simpan progress (anggap modul ini selesai)
        ...
        // kalau modul terakhir -> balik ke halaman course
        if ($isLastModule) {
            return redirect()
                ->route('skills.show', $slug)
                ->with('success', 'Mantap! Kamu sudah menyelesaikan pelatihan ini 🎉');
        }

        // kalau belum -> lanjut modul berikutnya
        return redirect()
            ->route('skills.module', [$slug, $moduleIndex + 1])
